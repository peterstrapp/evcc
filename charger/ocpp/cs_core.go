...
// DataTransferUpdate, if set, is called with parsed current/voltage values from
// DataTransfer vendor messages (e.g. MasterPlug). It is intended to be registered
// by the `meter` package so this package does not need to import `meter`.
var DataTransferUpdate func(chargePointID string, current, voltage float64)
	...
	if request != nil {
		...
		if request.VendorId == "MasterPlug" && request.MessageId == "GetCTClampValue" {
			...
			if cur, volt, err := meterParseMasterplug(inner); err == nil {
				...
				if DataTransferUpdate != nil {
					DataTransferUpdate(id, cur, volt)
				}
			} else {
